...
part <- last %>% filter(gamma == 8)
part <- part %>% mutate(value_new = round(value))

max(part$value - part$value_new)

part %>% ggplot(aes(x = beta, y = value_new, fill = name)) + geom_area() +
  facet_grid(cols = vars(lambda), labeller = label_both) +
  ylab('Fraction') + xlab('beta') + labs(fill = 'Compartment') +
  scale_x_continuous(trans = 'log10')

part %>% ggplot(aes(x = beta, y = value, fill = name)) + geom_area() +
  facet_grid(cols = vars(lambda), labeller = label_both) +
  ylab('Fraction') + xlab('beta') + labs(fill = 'Compartment') +
  scale_x_continuous(trans = 'log10')
