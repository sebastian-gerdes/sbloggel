...
* I couldn't install it via conda, so I created a new environment and installed it via pip:
```{r}
#| eval: false
conda_create('tf')
use_condaenv('tf')
conda_install('tf', c('tensorflow', 'tensorflow_decision_forests'), pip = TRUE)
conda_install('tf', c('matplotlib', 'seaborn'), pip = TRUE)
```
